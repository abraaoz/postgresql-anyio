...
def postgres_socket_file():
    ...
    with TemporaryDirectory() as data_dir:
        ...
        pg_options = (
            f'-F -c listen_addresses="" -c unix_socket_directories={socket_dir} '
        )
        start_cmd = [
            pg_ctl,
            "-D",
...
async def pool(postgres_socket_file, request):
    ...
    async with pganyio.create_pool(
        "testdb",
        username=username,
        protocol_format=fmt,
...
def find_pg_ctl():
    ...
    if not prefix.exists():
        raise RuntimeError(f"PostgreSQL directory {prefix} does not exist.")
    ...
    if not version_dirs:
        raise RuntimeError(f"No version directories found in {prefix}")

    _, highest_version_dir = version_dirs[-1]
    pg_ctl = highest_version_dir / "bin/pg_ctl"

    if not pg_ctl.exists():
        raise RuntimeError(f"pg_ctl not found at the expected location: {pg_ctl}")
