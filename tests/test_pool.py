...
async def test_concurrent_queries(pool):
  async with pool.acquire() as conn:
    await conn.execute("create table foobar (foo int unique, bar int not null)")
    await conn.execute("insert into foobar (foo, bar) values ($1, $2)", 101, 201)
...
async def test_prepared_stmt_concurrent(pool):
  ...
  async def perform(i):
    async with pool.acquire() as conn:
      ...
      async with conn.transaction() as _tr:
  ...
  async def task(i):
    async with pool.acquire() as conn:
      async with conn.transaction():
        await conn.execute("insert into foobar (foo) values ($1)", i + 0)
        await conn.execute("insert into foobar (foo) values ($1)", i + 1)
        await conn.execute("insert into foobar (foo) values ($1)", i + 2)

        async with conn.transaction():
          await conn.execute("insert into foobar (foo) values ($1)", i + 3)
          async with conn.transaction():
            await conn.execute("insert into foobar (foo) values ($1)", i + 4)
            await conn.execute("insert into foobar (foo) values ($1)", i + 5)
            async with conn.transaction() as sp:
              await conn.execute("insert into foobar (foo) values ($1)", 1000)
              await sp.rollback()
          await conn.execute("insert into foobar (foo) values ($1)", i + 6)

        await conn.execute("insert into foobar (foo) values ($1)", i + 7)
        await conn.execute("insert into foobar (foo) values ($1)", i + 8)
        await conn.execute("insert into foobar (foo) values ($1)", i + 9)
