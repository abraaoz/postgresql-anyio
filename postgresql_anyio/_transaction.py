...
class Transaction:
  def __init__(self, conn, *, isolation_level=None, read_write_mode=None, deferrable=False):
    self.conn = conn
  ...
  async def _commit(self):
    if self._savepoint_id is None:
      ...
      await self.conn.execute(f"release savepoint {self._savepoint_id}")

  async def _rollback(self):
    if self._savepoint_id is None:
      await self.conn._execute_simple("rollback")
    else:
      await self.conn._execute_simple(f"rollback to {self._savepoint_id}")
    ...
    if self.conn.current_transaction:
      ...
      if self.conn.in_transaction:
        raise InterfaceError(
          "Cannot start a transaction block inside a manually started transaction."
        )
