# pyright: reportAttributeAccessIssue=none, reportArgumentType=none, reportUnboundVariable=none
import hashlib
...
class _Int(int, PgBaseDataType):
  def __bytes__(self):
    return self.to_bytes(length=self._int_size, byteorder="big", signed=True)
...
class PgMessageMetaClass(type):
  def __new__(cls, name, bases, attrs, **kwargs):
    if name != "PgMessage":
      klass = PgMessageMetaClass._create_class(cls, name, bases, attrs, **kwargs)
    else:
  ...
  @staticmethod
  def _create_class(cls, name, bases, attrs, **kwargs):  # type: ignore
    if "_type" not in attrs:
      ...
      for base_class in bases:
        ...
        raise TypeError(f'Class {name} missing class variable "_type"')
    else:
      _type = attrs["_type"]

    if "side" not in kwargs:
      raise TypeError(f'Class {name} missing class keyword argument "side".')
    side = kwargs["side"]

    if _type is not None:
      if not isinstance(_type, bytes) or len(_type) != 1:
        raise ValueError("_type field should contain a bytes object of size 1 or None.")

    if side not in ["backend", "frontend", "both"]:
      raise ValueError(
        'Class argument "side" can only have one of these values: backend, frontend, both'
      )
    ...
    for attr, value in attrs.items():
      if attr.startswith("_"):
        continue
      if (
        not isinstance(value, PgBaseDataType)
        and not (isinstance(value, type) and issubclass(value, PgBaseDataType))
    ...
    klass = super().__new__(cls, name, bases, attrs)  # type: ignore
...
class PgMessage(metaclass=PgMessageMetaClass):
  ...
  def __bytes__(self):
    ...
    for attr, field in vars(klass).items():
      ...
      if isinstance(field, PgBaseDataType):
        ...
        value = bytes(field)
      elif isinstance(field, type) and issubclass(field, PgBaseDataType):
  ...
  def _deserialize(cls, msg, start, length):
    ...
    for attr, value in vars(cls).items():
      ...
      elif isinstance(value, type) and issubclass(value, PgBaseDataType):
        # the field contains just a type (like Int32)
        field_type = value
      elif callable(value):
        raise ValueError("Cannot deserialize messages with dynamic fields.")
      else:
...
class BackendKeyData(PgMessage, side="backend"):
  ...
  def __repr__(self):
    return f"<BackendKeyData pid={self.pid} secret_key={self.secret_key}>"
...
class Bind(PgMessage, side="frontend"):
  ...
  def __init__(
    self, portal_name, stmt_name, params=None, param_format_codes=None, result_format_codes=None
  ):
  ...
  def __bytes__(self):
    ...
    param_format_code_count = bytes(Int16(len(self.param_format_codes)))
    param_format_codes = b"".join(bytes(Int16(fc)) for fc in self.param_format_codes)
    param_count = bytes(Int16(len(self.params)))
    params = b"".join(
      # NULL values are indicated by setting the length field to
      # -1
      (bytes(Int32(len(p) if p is not None else -1)) + (b"" if p is None else p))
      for p in self.params
    )
    result_format_code_count = bytes(Int16(len(self.result_format_codes)))
    result_format_codes = b"".join(bytes(Int16(fc)) for fc in self.result_format_codes)
    length = bytes(
      Int32(
        4
        + len(portal_name)
  ...
  def __repr__(self):
    ...
    return f"<Bind portal={self.portal_name} stmt={self.stmt_name}{params}{pcodes}{rcodes}>"
...
class ErrorResponse(PgMessage, side="backend"):
  ...
  def _deserialize(cls, msg, start, length):
    ...
    while True:
      if idx >= len(msg):
        raise ValueError("Unterminated ErrorResponse message (should end with a zero byte)")
...
class NoticeResponse(PgMessage, side="backend"):
  ...
  def __repr__(self):
    ...
    notices = " ".join(f"{code}={value}" for code, value in self.notices)
  ...
  def _deserialize(cls, msg, start, length):
    ...
    while True:
      if idx >= len(msg):
        raise ValueError("Unterminated NoticeMessage message (should end with a zero byte)")
      code, n = Byte1.deserialize(msg, idx)
      if code == b"\0":
        break
      code = chr(code[0])  # convert e.g. b'C' to 'C'
...
class ParameterStatus(PgMessage, side="backend"):
  ...
  def __repr__(self):
    return f"<ParameterStatus name={self.param_name} value={self.param_value}>"
...
class Parse(PgMessage, side="frontend"):
  ...
  def __bytes__(self):
    ...
    param_types = b"".join(bytes(Int32(pt)) for pt in self.param_types)
    param_type_count = bytes(Int16(len(self.param_types)))
    length = bytes(Int32(4 + len(name) + len(query) + len(param_type_count) + len(param_types)))
    return b"P" + length + name + query + param_type_count + param_types
  ...
  def __repr__(self):
    ...
    return f'<Parse name={name} query="{query}" param_types={self.param_types}>'
...
class PasswordMessage(PgMessage, side="frontend"):
  ...
  def __init__(self, password, *, md5=False, username=None, salt=None):
    ...
    if md5:
      ...
      userpass_md5 = hashlib.md5(userpass).hexdigest().encode("ascii")
      final_hash = hashlib.md5(userpass_md5 + salt).hexdigest().encode("ascii")
...
class Query(PgMessage, side="frontend"):
  ...
  def __init__(self, query):
    ...
    if not isinstance(query, bytes):
      raise ValueError(f"query should either by a str or bytes; got {type(query)}")
...
class RowDescription(PgMessage, side="backend"):
  ...
  def _deserialize(cls, msg, start, length):
    ...
    while len(obj.fields) < obj.nfields:
      ...
      idx += n
      obj.fields.append(
        (name, table_oid, column_attr_id, type_oid, type_len, type_modifier, format_code)
      )
    return obj
