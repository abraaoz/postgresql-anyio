...
class Pool:
  def __init__(
    ...
    **connection_kwargs,
    ...
    if pool_max_size < pool_min_size:
      raise ValueError("pool_max_size cannot be less than pool_min_size")
    ...
    self._nursery = None
    self._conn_limit = anyio.CapacityLimiter(self.max_size)
  ...
  async def acquire(self):
    async with self._conn_limit:
      while True:
        ...
        if not conn.closed.is_set() and not conn._start_closing.is_set():
          break
  ...
  async def _run(self):
    async with anyio.create_task_group() as nursery:
      ...
      with anyio.move_on_after(self.close_timeout):
        for conn in all_conns:
  ...
  async def _create_connection(self):
    ...
    conn = Connection(self.database, codec_helper=self._codec_helper, **self._conn_kwargs)
