...
from ._text_array import ArrayParseError, parse_text_array
...
class CodecHelper:
  ...
  def register_codec(self, codec):
    if not isinstance(codec, type) or not issubclass(codec, Codec):
      raise TypeError("Codec should be a sub-class of Codec class")
  ...
  def encode_value(self, value, type_oid, protocol_format):
    ...
    return self.encode_single_value(value, type_oid, protocol_format)
  ...
  def encode_single_value(self, value, type_oid, protocol_format):
    ...
    if type_name is None:
      raise InterfaceError(f'No codec to encode parameter "{value}" to type "{type_name}"')
  ...
  def encode_array(self, value, elem_oid, protocol_format):
    if protocol_format == PgProtocolFormat.BINARY:
      return self.encode_array_binary(value, elem_oid, protocol_format)
    else:
      return self.encode_array_text(value, elem_oid, protocol_format).encode("utf-8")

  def encode_array_text(self, value, elem_oid, protocol_format):
    if not isinstance(value, list):
      raise TypeError(f"Expected a list for a postgres array; Got: {value!r}")
    ...
    for elem in value:
      ...
      if isinstance(elem, list):
        ret += self.encode_array_text(elem, elem_oid, protocol_format)
      else:
        encoded_elem = self.encode_value(elem, elem_oid, protocol_format)
  ...
  def encode_array_binary(self, value, elem_oid, protocol_format):
    ...
    for elem in flatten(value):
      encoded_value = self.encode_value(elem, elem_oid, protocol_format)
  ...
  def check_array_dims(self, array, dims=None):
    ...
    if all(isinstance(e, list) for e in array):
      ...
      for subarr in array[1:]:
        ...
        if dims2 != dims1:
          raise ValueError("Multidimensional arrays must have sub-arrays with matching dimensions")

      return dims1
    elif not any(isinstance(e, list) for e in array):
      return dims
    else:
      raise ValueError("Each array level should either be all other arrays, or all non-arrays.")
  ...
  def decode_row(self, columns, row_desc, tuple_class=None):
    ...
    return tuple_class(
      *(self.decode_col(col, col_desc) for col, col_desc in zip(columns, row_desc))
    )
  ...
  def decode_col(self, col, col_desc):
    ...
    (name, table_oid, column_attr_id, type_oid, type_len, type_modifier, format_code) = col_desc
  ...
  def decode_value(self, value, type_oid, format_code):
    type_name = self._oid_to_name.get(type_oid)
    if type_name is None:
      warnings.warn(f"Unknown type OID: {type_oid}; returning un-decoded data.")
      return value

    codec = self._type_name_to_codec.get(type_name)
    if codec is None:
      warnings.warn(f"Unknown postgres type: {type_name}; returning un-decoded data.")
      return value
    ...
    if format_code == PgProtocolFormat.TEXT:
      ...
      raise InterfaceError(f"Unexpected column format code: {format_code}")

  def decode_array(self, value, elem_oid, format_code):
    if format_code == PgProtocolFormat.BINARY:
      return self.decode_array_binary(value, elem_oid, format_code)
    else:
      return self.decode_array_text(value, elem_oid, format_code)

  def decode_array_binary(self, value, elem_oid, format_code):
    buf = memoryview(value)

    ndims = read_int(buf, 0)
    _flags = read_int(buf, 4)
    ...
    if ndims > ARRAY_MAX_DIM:
      raise InterfaceError(
        f"Array dimensions greater than expected max. Expected up to {ARRAY_MAX_DIM}; Got: {ndims}"
      )
    elif ndims < 0:
    ...
    for i in range(array_length):
      ...
      if elem_length < 0:
        ...
        elem = self.decode_value(bytes(buf[idx : idx + elem_length]), elem_oid, format_code)
        idx += elem_length
  ...
  def decode_array_text(self, value, elem_oid, format_code):
    ...
    try:
      ...
      raise InterfaceError(f"Received invalid array literal: '{value}'")
    array = self.decode_parsed_text_array(array, elem_oid, format_code)
    return array
  ...
  def decode_parsed_text_array(self, array, elem_oid, format_code):
    ...
    for e in array:
      if isinstance(e, str):
        ...
        ret.append(self.decode_parsed_text_array(e, elem_oid, format_code))
      elif e is None:
        ret.append(None)
      else:
        raise InternalError(f"Expected string in parsed array; got: {e!r}")
    return ret
...
class CodecMetaclass(type):
  def __new__(cls, name, bases, attrs, **kwargs):
    ...
    if not isinstance(klass.pg_type, str):
      raise TypeError(f"pg_type field must be a string; Got: {klass.pg_type!r}")
    ...
    if hasattr(klass, "python_types"):
      ...
      if not all(isinstance(t, type) for t in klass.python_types):
        raise TypeError(
          f"Codec class python_types field should contain only types; Got: {klass.python_types}"
        )
    ...
    for method_name in expected_methods:
      if not hasattr(klass, method_name):
        raise TypeError(f"Method {method_name} not found in codec class {name}")
      from inspect import getattr_static

      method = getattr_static(klass, method_name)
      if not isinstance(method, classmethod) and not isinstance(method, staticmethod):
        raise TypeError(
          "Codec encode/decode methods should either be class methods or static methods."
        )
      ...
      if klass.python_types:
        setattr(
          ...
          cls.get_typechecked_method(
            klass, getattr(klass, method_name), klass.python_types, klass.pg_type
          ),
        )
  ...
  @classmethod
  def get_typechecked_method(cls, codec_class, method, types, pg_type_name):
    @wraps(method)
    ...
    def decode_wrapper(value, *args, **kwargs):
      if method.__name__ == "decode_binary":
        if not isinstance(value, (bytes, memoryview)):
          raise TypeError(f"decode_binary expects a bytes value; Got: {value!r}")
      else:
        if not isinstance(value, str):
          raise TypeError(f"decode_text expects a string value; Got: {value!r}")
      result = method(value, *args, **kwargs)
      if not isinstance(result, types):
        expected_types = f"[{', '.join(t.__name__ for t in types)}]"
        raise TypeError(
...
class _Int(Codec):
  ...
  def encode_binary(cls, value):
    return value.to_bytes(length=cls._int_length, byteorder="big", signed=True)
...
class Interval(Codec):
  ...
  def decode_text(cls, value):
    ...
    values = {}
    for i in ["years", "months", "weeks", "days", "hours", "minutes", "seconds"]:
...
class TimeTz(Codec):
  ...
  def encode_binary(cls, value):
    ...
    zone = zone.to_bytes(length=4, byteorder="big", signed=True)
    useconds = useconds.to_bytes(length=8, byteorder="big", signed=True)
...
def decode_inet_or_cidr(value):
  ...
  if family == PGSQL_AF_INET:
    ...
    raise InterfaceError("Invalid address family code received: {family}")

  if bits > max_prefixlen:
    raise InterfaceError(f"Invalid IPv{ip_version} CIDR netmask received: /{bits}")

  if len(address) != address_length:
    raise InterfaceError("Invalid network address: address_length does not match length of data")
...
def read_int(data, idx=0):
  return int.from_bytes(data[idx : idx + 4], byteorder="big", signed=True)
...
def read_text_array(buf, idx=0):
  ...
  if len(buf) - idx < 2:
    raise ValueError(f"Array literal should be at least 2 characters; '{buf}'")
  ...
  if next_char() != "{":
    raise InterfaceError(
      f"Invalid array literal received; expected \"{{\" at index {idx - 1}: '{buf}'"
    )
  ...
  while idx < len(buf):
    ...
    if c == ",":
      if cur_item:
        ...
        raise InterfaceError(
          f"Invalid array literal: empty element before ',' at index {idx - 1}: {buf}"
        )
      continue
  ...
  if quoted:
    raise InterfaceError(f"Unexpected end of string literal inside array literal: '{buf}'")
