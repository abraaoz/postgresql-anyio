...
from ._exceptions import InterfaceError, ProgrammingError
...
class PreparedStatement:
  ...
  async def _parse_query(self):
    ...
    await self.conn._send_msg(parse_msg, describe_stmt_msg, sync_msg)
    ...
    while True:
      ...
      if isinstance(msg, _pgmsg.ErrorResponse):
        ...
        raise get_exc_from_msg(
          msg,
          desc_prefix=(f"Error preparing statement: {self.query}\n   "),
        )
  ...
  def parameters(self):
    ...
    return [
      ParameterDesc(name=name, type=self.conn._codec_helper._oid_to_name.get(type_oid))
      for name, table_oid, column_attr_id, type_oid, type_len, type_modifier, format_code in self._row_desc
    ]
  ...
  async def execute(self, *params, limit=None):
    if not self._initialized:
      raise ProgrammingError("PreparedStatement not initialized")
    ...
    if limit and limit is not NO_EXECUTE and not self.conn.in_transaction:
      ...
      raise InterfaceError("limit parameter not supported outside a transaction")
  ...
  async def _execute(self, *params, limit=None):
    ...
    enc_params = []
    for param, param_oid in zip(params, self._param_oids or []):
      param_value = self.conn._codec_helper.encode_value(
        param, param_oid, protocol_format=protocol_format
      )
      enc_params.append(param_value)

    bind_msg = _pgmsg.Bind(
      self._portal_name,
      self._stmt_name,
      params=enc_params,
    ...
    if limit is not NO_EXECUTE:
      ...
      msgs = [bind_msg, describe_portal_msg, execute_msg, sync_msg]
    else:
    ...
    while True:
      ...
      if isinstance(msg, _pgmsg.ErrorResponse):
        raise get_exc_from_msg(
          msg,
          desc_prefix=(f"Error executing statement: {self.query}\n   "),
        )
      elif isinstance(msg, _pgmsg.DataRow):
        row = self.conn._codec_helper.decode_row(msg.columns, self._row_desc, self.tuple_class)
        results.append(row)
      elif isinstance(msg, _pgmsg.RowDescription):
        self._row_desc = msg.fields
  ...
  async def exec_continue(self, limit=None):
    ...
    if not self._execute_started:
      raise ProgrammingError("exec_continue can only be called after execute is called")

    if not self.conn.in_transaction:
      # we don't allow this because outside a transaction
      # postgres will complain that "portal does not exist"
      raise InterfaceError("Cannot continue execution of a statement outside a transaction")

    if self.conn.current_transaction != self._initial_transaction:
      raise ProgrammingError("Cannot continue execution from another transaction")
  ...
  async def _exec_continue(self, limit=None):
    ...
    while True:
      msg = await self.conn._get_msg(
        _pgmsg.ErrorResponse, _pgmsg.DataRow, _pgmsg.CommandComplete, _pgmsg.PortalSuspended
      )
      if isinstance(msg, _pgmsg.DataRow):
        row = self.conn._codec_helper.decode_row(msg.columns, self._row_desc)
        results.append(row)
        ...
        if cast(bytes, msg.cmd_tag).startswith(b"SELECT"):
          _, rows = cast(bytes, msg.cmd_tag).split(b" ")
  ...
  async def _close_portal(self):
    ...
    msg = await self.conn._get_msg(_pgmsg.CloseComplete, _pgmsg.ErrorResponse)
    if isinstance(msg, _pgmsg.ErrorResponse):
      raise get_exc_from_msg(
        msg,
        desc_prefix=(f"Error closing portal: {self.query}\n   "),
      )
  ...
  async def forward(self, n):
    ...
    await self.conn.execute(f"move forward {n} from {self._portal_name}")
